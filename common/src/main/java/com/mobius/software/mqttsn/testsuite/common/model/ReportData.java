...
public class ReportData
{
	...
	private ConcurrentHashMap<SNType, AtomicInteger> inPacketCounters = new ConcurrentHashMap<>();
	private ConcurrentHashMap<SNType, AtomicInteger> outPacketCounters = new ConcurrentHashMap<>();
	...
	{
		Map<SNType, Counters> map = new HashMap<>();
		for (Entry<SNType, AtomicInteger> entry : inPacketCounters.entrySet())
		{
		...
		for (Entry<SNType, AtomicInteger> entry : outPacketCounters.entrySet())
		{
			if (entry.getValue().get() == 0)
				continue;
	...
	public void countIn(SNType type)
	{
		inPacketCounters.get(type).incrementAndGet();
	}

	public void countOut(SNType type)
	{
	...
	public ConcurrentHashMap<SNType, AtomicInteger> getInPacketCounters()
	{
		return inPacketCounters;
	}

	public ConcurrentHashMap<SNType, AtomicInteger> getOutPacketCounters()
	{
